...
election22tpp <- data.frame(c("LNP","ALP"), c(45.10,54.59))
...
primary_votes <- ggplot(polling2226, aes(x=as.Date(last_date, '%d %b %Y'))) +
  ...
  geom_point(data = election22, aes(x = as.Date('2022-03-19', '%Y-%m-%d'), y = vote, colour = party), shape=23, stroke=0.5, fill = "#FFFFFF", size=4) +
  geom_point(data = election22, aes(x = as.Date('2022-03-19', '%Y-%m-%d'), y = vote, colour = party), shape=18, size=3) +
  ...
  scale_x_date(limits=c(as.Date('2022-03-19', '%Y-%m-%d'), max_date), date_breaks = "1 month", date_labels = "%b %Y", minor_breaks = "1 month", expand = c(0,0)) +
...
tpp <- ggplot(polling2226, aes(x=as.Date(last_date, '%d %b %Y'))) +
  ...
  geom_point(data = election22tpp, aes(x = as.Date('2022-03-19', '%Y-%m-%d'), y = vote, colour = party), shape=23, stroke=0.5, fill = "#FFFFFF", size=4) +
  geom_point(data = election22tpp, aes(x = as.Date('2022-03-19', '%Y-%m-%d'), y = vote, colour = party), shape=18, size=3) +
